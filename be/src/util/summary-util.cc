...
void PrintExecSummary(const TExecSummary& exec_summary, int indent_level,
    bool is_child_fragment, int* node_idx, vector<vector<string> >* result) {
...
#undef COMPUTE_MAX_SUM_STATS
  ...
  if (child_fragment_idx_it != exec_summary.exch_to_sender_map.end()) {
    ...
    PrintExecSummary(exec_summary, indent_level, true, &child_fragment_id, result);
  }
  ++*node_idx;
  ...
  for (int i = 1; i < node.num_children; ++i) {
    PrintExecSummary(exec_summary, indent_level + 1, false, node_idx, result);
  }
