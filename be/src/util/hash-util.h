...
/// CRC32 checksum implementation using table-based lookup.
/// Modified version of
/// https://stackoverflow.com/questions/24982561/diffrent-result-for-crc32-in-c-and-java
/// generated by Claude Sonnet 4.5
///
/// This implementation produces checksums compatible with Java's standard
/// java.util.zip.CRC32 implementation
///
/// NOTE: This is different from HashUtil::CrcHash() which:
/// - Uses hardware instructions for performance in hash table operations
/// - Swaps hash output halves for better hash distribution
/// - Is optimized for hash table bucketing, not standard CRC32 checksums
///
/// Use CRC32::ComputeChecksum() when you need:
/// - Standard CRC32 checksums for data integrity verification
/// - Compatibility with external systems expecting standard CRC32
/// - Checksums that match other language implementations
///
/// Use HashUtil::CrcHash() when you need:
/// - Fast hashing for internal hash tables and data structures
/// - Hardware-accelerated hash computation
namespace CRC32 {
  /// Compile-time helper to compute a single CRC32 table entry.
  /// This function is evaluated at compile time to generate the lookup table.
  constexpr uint32_t ComputeCrc32TableEntry(uint32_t index) {
    constexpr uint32_t poly = 0xedb88320;  // Standard CRC32 polynomial
    uint32_t temp = index;
    for (int j = 8; j > 0; --j) {
      if ((temp & 1) == 1) {
        temp = (temp >> 1) ^ poly;
  ...
  constexpr std::array<uint32_t, 256> MakeCrc32Table(std::index_sequence<Indices...>) {
    return {{ComputeCrc32TableEntry(Indices)...}};
  }

  /// Precomputed CRC32 lookup table generated at compile time.
  inline constexpr auto kCrc32Table = MakeCrc32Table(std::make_index_sequence<256>{});

  /// Computes the standard CRC32 checksum of the given data.
  /// @param bytes Pointer to the data to checksum
  /// @param size Number of bytes to checksum
  /// @return The CRC32 checksum value
  inline uint32_t ComputeChecksum(const uint8_t* bytes, size_t size) {
