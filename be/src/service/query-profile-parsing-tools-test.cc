...
constexpr const char* NON_JSON_PROFILE_INPUT = "not a json query profile";
...
TEST(QueryProfileParsingToolsTest, ToolMethodsReturnExpectedProfileSections) {
  ...
  for (const ProfileCase& profile : profiles) {
    ...
    Status executor_status =
        CreateQueryProfileToolExecutorForProfile(profile_text, &tool_executor);
    ...
    for (const ToolCase& tc : tools) {
      ...
      string output_json;
      const Status status = tool_executor(tc.tool_name, tool_args, &output_json);
      ...
      if (update_goldens) {
        const string pretty_json = FormatJson(output_json, /*pretty=*/true);
      ...
      EXPECT_EQ(FormatJson(expected_json, /*pretty=*/false),
          FormatJson(output_json, /*pretty=*/false))
          << "profile=" << profile.profile_file
...
TEST(QueryProfileParsingToolsTest, ToolMethodsRejectNonJsonProfileInput) {
  QueryProfileToolExecutor tool_executor;
  const Status status =
      CreateQueryProfileToolExecutorForProfile(NON_JSON_PROFILE_INPUT, &tool_executor);
...
TEST(QueryProfileParsingToolsTest, ToolExecutorRejectsInputLargerThanConfiguredLimit) {
  ...
  const Status status = CreateQueryProfileToolExecutorForProfile(
      profile_text, &tool_executor, /*profile_size_limit_bytes=*/32);
  EXPECT_FALSE(status.ok());
  EXPECT_STR_CONTAINS(
...
TEST(
  ...
  const Status create_status = CreateQueryProfileToolExecutorForProfile(
      profile_text, &tool_executor, /*profile_size_limit_bytes=*/1024);
  ASSERT_TRUE(create_status.ok()) << create_status.GetDetail();

  string output_json;
  const Status execute_status = tool_executor("get_summary", "{}", &output_json);
  ...
  constexpr const char* profile_text = R"json(
{
  ...
  const Status run_status = tool_executor(
      "get_operator", R"({"node_id":"1","fragment_id":"F00"})", &output_json);
  ASSERT_TRUE(run_status.ok()) << run_status.GetDetail();

  rapidjson::Document node_doc;
  node_doc.Parse(output_json.c_str());
  ASSERT_FALSE(node_doc.HasParseError()) << output_json;
  ASSERT_TRUE(node_doc.IsObject()) << output_json;
  ASSERT_TRUE(node_doc.HasMember("id")) << output_json;
  ASSERT_TRUE(node_doc["id"].IsString()) << output_json;
  ...
  constexpr const char* profile_text = R"json(
{
  ...
  string averaged_json;
  const Status averaged_status =
      tool_executor("get_average_operator", R"({"node_id":"45"})", &averaged_json);
  ASSERT_TRUE(averaged_status.ok()) << averaged_status.GetDetail();
  rapidjson::Document averaged_doc;
  averaged_doc.Parse(averaged_json.c_str());
  ASSERT_FALSE(averaged_doc.HasParseError()) << averaged_json;
  ...
  string all_json;
  const Status all_status =
      tool_executor("get_all_operators", R"({"node_id":"45","fragment_id":"F00"})",
          &all_json);
  ASSERT_TRUE(all_status.ok()) << all_status.GetDetail();
  rapidjson::Document all_doc;
  all_doc.Parse(all_json.c_str());
  ASSERT_FALSE(all_doc.HasParseError()) << all_json;
  ...
  string specific_json;
  const Status specific_status = tool_executor("get_specific_operator",
      R"({"node_id":"45","fragment_id":"F00","instance_id":"inst2"})",
      &specific_json);
  ASSERT_TRUE(specific_status.ok()) << specific_status.GetDetail();
  rapidjson::Document specific_doc;
  specific_doc.Parse(specific_json.c_str());
  ASSERT_FALSE(specific_doc.HasParseError()) << specific_json;
  ...
  string fragment_json;
  const Status fragment_status =
      tool_executor("get_fragment", R"({"fragment_id":"F00"})", &fragment_json);
  ASSERT_TRUE(fragment_status.ok()) << fragment_status.GetDetail();
  rapidjson::Document fragment_doc;
  fragment_doc.Parse(fragment_json.c_str());
  ASSERT_FALSE(fragment_doc.HasParseError()) << fragment_json;
  ...
  string all_fragments_json;
  const Status all_fragments_status =
      tool_executor("AllFragments", R"({"fragment_id":"F00"})", &all_fragments_json);
  ASSERT_TRUE(all_fragments_status.ok()) << all_fragments_status.GetDetail();
  rapidjson::Document all_fragments_doc;
  all_fragments_doc.Parse(all_fragments_json.c_str());
  ASSERT_FALSE(all_fragments_doc.HasParseError()) << all_fragments_json;
