...
DEFINE_bool(allow_catalog_cache_op_from_masked_users, true, "Whether to allow table "
    ...
    "considered as read-only users. Set this to false to block such operations.");
...
Frontend::Frontend() {
  JniMethodDescriptor methods[] = {
    ...
    {"finalizeIcebergDml", "([B)V", &finalize_iceberg_dml_id_},
    ...
    {"getPrincipalRoles", "([B)[B", &get_principal_roles_id_},
...
Status Frontend::FinalizeIcebergDml(const TIcebergDmlFinalizeRequest& req) {
  return JniUtil::CallJniMethod(fe_, finalize_iceberg_dml_id_, req);
}
...
Status Frontend::GetPrincipalRoles(const TShowRolesParams& params,
    TResultSet* result) {
  return JniUtil::CallJniMethod(fe_, get_principal_roles_id_, params, result);
}
