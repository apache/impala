...
HdfsScanNodeBase::HdfsScanNodeBase(ObjectPool* pool, const HdfsScanPlanNode& pnode,
  ...
  : ScanNode(pool, pnode, descs),
    ...
    clear_file_path_slot_(hdfs_scan_node.__isset.clear_file_path_slot ?
            hdfs_scan_node.clear_file_path_slot :
            false),
...
Status HdfsScanNodeBase::CreateAndOpenScannerHelper(
    ScannerContext* context, scoped_ptr<HdfsScanner>* scanner) {
  DCHECK(context != nullptr);
  DCHECK(scanner->get() == nullptr);

  const HdfsFileDesc* file_desc = context->GetStream(0)->file_desc();
  THdfsCompression::type compression = file_desc->file_compression;

  switch (file_desc->file_format) {
    case THdfsFileFormat::TEXT:
    ...
    default:
      return Status(
          Substitute("Unknown Hdfs file format type: $0", file_desc->file_format));
  }
  DCHECK(scanner->get() != nullptr);
