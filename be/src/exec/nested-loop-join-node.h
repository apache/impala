...
class NestedLoopJoinNode : public BlockingJoinNode {
 ...
 private:
  ...
  /// The ownership of these batches is transfered to the output batch in
  ...
  /// Retrieves the next probe row from the left child. The set_eos_when_done flag
  /// specifies if more result rows are to be produced when all the probe side
  /// rows have been processed. When set_eos_when_done is 'true', the join node
  /// cannot produce more result tuples when the probe side is exchausted.
  Status NextProbeRow(RuntimeState* state, RowBatch* output_batch,
      bool set_eos_when_done);
  ...
  /// Returns true if there are more probe rows to be processed, false
  /// otherwise.
  bool HasMoreProbeRows() {
    DCHECK(probe_batch_pos_ != 0 || probe_side_eos_);
    return probe_batch_pos_ != 0;
  }
};
