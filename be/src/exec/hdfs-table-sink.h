...
class HdfsTableSink : public DataSink {
 ...
 private:
  ...
  // Files are created in a temporary directory and then either the files or the
  // whole directory, in the case of an overwrite, are moved to the real partition
  // directory.
  Status CreateNewTmpFile(OutputPartition* output_partition);
