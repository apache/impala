...
template <int buffer_bytes, int streaming_lines>
class HashStore {
 public:
  HashStore() {
    num_tables_ = 1;
    tables_ = new StandardHashTable[num_tables_];
    buffers_ = AllocateBuffers(num_tables_);
  ...
  ~HashStore() {
    delete[] tables_;
    // Note: Won't call any constructor.
    free(buffers_);
 ...
 private:
  ...
  static const int BUFFER_BUFFER_SIZE = 8 * streaming_lines;
  ...
  Buffer* AllocateBuffers(int n) {
    Buffer* buffers;
    bool fail = posix_memalign((void**)&buffers, __alignof(*buffers), sizeof(*buffers) * n);
    CHECK(!fail);
    CHECK_EQ(((long)buffers) % 64, 0);
    for (int i = 0; i < n; ++i) {
  ...
  void BufferTuple(const ProbeTuple* tuple) {
    ...
    if (buffer_buffer.Full()) {
      ...
      if (LIKELY(mod == 0)) {
        ...
        DCHECK_EQ(buffer.count % BUFFER_BUFFER_SIZE, 0);
