...
class HashStore {
 public:
  typedef ProbeTuple BufferedTuple;
  ...
  void Aggregate(BufferedTuple* tuples, uint64_t n) {
    for (uint64_t i = 0; i < n; ++i) {
      BufferedTuple* tuple = &tuples[i];
      ComputeHash(tuple);
      if (buffer_bytes == 0) {
 ...
 private:
  ...
  static const int BUFFER_SIZE = buffer_bytes == 0 ? 0 : TUPLE_BYTES / sizeof(BufferedTuple);
  static const int PADDING_BYTES = buffer_bytes == 0 ? 0 : TUPLE_BYTES % sizeof(BufferedTuple);
  ...
  struct Buffer {
    ...
    BufferedTuple tuples[BUFFER_SIZE];
  ...
  void DoubleAndRehash() {
    ...
    for (int b = 0; b < old_num_tables; ++b) {
      ...
      for (int i = old_buffer.start; i < old_buffer.count; ++i) {
        const BufferedTuple* tuple = &old_buffer.tuples[i];
        uint32_t idx = IndexFor(tuple->hash);
        Buffer& buffer = buffers_[idx];
  ...
  inline bool ProcessTuple(const BufferedTuple* probe, StandardHashTable* table) {
    ...
    if (existing != NULL) {
      insert = TupleCombine(probe, existing);
    }
    // We need to insert if we didn't find an equal BuildTuple, or if TupleCombine's
    // return value told us to.
    if (insert) {
      BuildTuple build;
      TupleConvert(probe, &build);
      doubled = PutTuple(&build, table);
    }
    return doubled;
  }
