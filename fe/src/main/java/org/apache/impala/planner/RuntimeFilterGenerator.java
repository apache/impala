...
public final class RuntimeFilterGenerator {
  ...
  // Set of already-visited CTEProducerNode ids to avoid processing the same producer
  // more than once when multiple CTEConsumerNodes share the same producer.
  private final Set<PlanNodeId> visitedCTEProducers_ = new HashSet<>();

  // Filters registered outside the current CTE producer scope that must not be assigned
  // to scan nodes inside the producer, since the producer materializes before join
  // builds execute. Maps filter ID to a reference count to handle nested CTEs.
  private final Map<RuntimeFilterId, Integer> cteProducerBlockedFilters_ =
      new HashMap<>();
  ...
  public static class RuntimeFilter {
    ...
    private static class RuntimeFilterTarget {
      // Plan node that applies the filter (ScanNode or CTEConsumerNode)
      ...
      public RuntimeFilterTarget(CTEConsumerNode targetNode, Expr targetExpr,
          boolean isLocalTarget) {
        Preconditions.checkState(targetExpr.isBoundByTupleIds(targetNode.getTupleIds()));
        node = targetNode;
        expr = targetExpr;
        this.isBoundByPartitionColumns = false;
        this.isColumnInDataFile = false;
        this.isLocalTarget = isLocalTarget;
        this.lowValue = null;
        this.highValue = null;
      }
    ...
    /**
     * Static function that returns the ids of slots bound by base table tuples (or CTE
     * consumer output tuples) for which there is a value transfer from 'srcSid'. The
     * slots are grouped by tuple id.
     */
    private static Map<TupleId, List<SlotId>> getBaseTblEquivSlots(Analyzer analyzer,
      ...
      for (SlotId targetSid: analyzer.getValueTransferTargets(srcSid)) {
        ...
        if (tupleDesc.getTable() == null && !tupleDesc.isCTEOutput()) continue;
  ...
  public static void generateRuntimeFilters(PlannerContext ctx, PlanNode plan) {
    ...
    arrangeRuntimeFilters(plan);
  }

  public static void arrangeRuntimeFilters(
      PlanNode root) {
    if (root instanceof HdfsScanNode) {
      ((HdfsScanNode)root).arrangeRuntimefiltersForParquet();
    } else if (root instanceof CTEConsumerNode) {
      ((CTEConsumerNode)root).arrangeRuntimeFilters();
    }
    for (PlanNode childNode: root.getChildren()) {
      arrangeRuntimeFilters(childNode);
    }
  }
  ...
  private void generateFiltersRecursive(PlannerContext ctx, PlanNode root) {
    if (root instanceof HashJoinNode || root instanceof NestedLoopJoinNode) {
      ...
      // Assign pending filters to this CTE consumer, then recurse into the CTE
      // producer for any filters generated by joins inside the CTE itself.
      assignRuntimeFiltersToCTEConsumer(ctx, (CTEConsumerNode) root);
      for (PlanNode child : root.getChildren()) {
        if (child instanceof CTEProducerNode) {
          if (!visitedCTEProducers_.add(child.getId())) continue;
          // Block externally-registered filters so they are not re-assigned to scan
          // nodes inside the producer.
          Set<RuntimeFilterId> blocked = blockFiltersForCteProducer();
          generateFiltersRecursive(ctx, child);
          unblockFiltersForCteProducer(blocked);
        } else {
          generateFiltersRecursive(ctx, child);
        }
      }
  ...
  private void assignRuntimeFilters(PlannerContext ctx, ScanNode scanNode) {
    ...
    for (RuntimeFilter filter: runtimeFiltersByTid_.get(tid)) {
      ...
      if (cteProducerBlockedFilters_.containsKey(filter.getFilterId())) continue;
  ...
  /**
   * Check if 'targetNode' (any plan node) is local to the source node of 'filter'.
   */
  static private boolean isLocalTargetForPlanNode(
      RuntimeFilter filter, PlanNode targetNode) {
    if (targetNode.getFragment() == null) return false;
    return targetNode.getFragment().getId().equals(filter.src_.getFragment().getId());
  }

  /** Snapshots all currently registered filters as blocked inside a CTE producer.
   * Returns the blocked set so the caller can restore state via
   * unblockFiltersForCteProducer(). */
  private Set<RuntimeFilterId> blockFiltersForCteProducer() {
    Set<RuntimeFilterId> blocked = new HashSet<>();
    for (List<RuntimeFilter> filterList : runtimeFiltersByTid_.values()) {
      for (RuntimeFilter filter : filterList) blocked.add(filter.getFilterId());
    }
    for (RuntimeFilterId id : blocked) {
      cteProducerBlockedFilters_.merge(id, 1, Integer::sum);
    }
    return blocked;
  }

  /** Reverses the effect of a prior blockFiltersForCteProducer() call. */
  private void unblockFiltersForCteProducer(Set<RuntimeFilterId> blocked) {
    for (RuntimeFilterId id : blocked) {
      cteProducerBlockedFilters_.compute(id, (k, v) -> v == 1 ? null : v - 1);
    }
  }

  /**
   * Assigns pending runtime filters to a CTE consumer node. Filters targeting the CTE
   * consumer's output tuple are assigned to 'cteNode'. This is correct because the CTE
   * producer materializes data before the hash join build executes, so filters must be
   * applied at the consumer level when reading from the LocalExchanger. Assignment to
   * scan nodes inside the producer is prevented by the blocking mechanism in
   * generateFiltersRecursive().
   */
  private void assignRuntimeFiltersToCTEConsumer(PlannerContext ctx,
      CTEConsumerNode cteNode) {
    Preconditions.checkState(cteNode.getTupleIds().size() == 1);
    TupleId tid = cteNode.getTupleIds().get(0);
    if (!runtimeFiltersByTid_.containsKey(tid)) return;
    Analyzer analyzer = ctx.getRootAnalyzer();
    boolean disableRowRuntimeFiltering =
        ctx.getQueryOptions().isDisable_row_runtime_filtering();
    ...
    for (RuntimeFilter filter : runtimeFiltersByTid_.get(tid)) {
      if (filter.isFinalized()) continue;
      if (cteProducerBlockedFilters_.containsKey(filter.getFilterId())) continue;
      // CTE consumer filters are never partition-bound, so skip in row-filtering mode.
      if (disableRowRuntimeFiltering) continue;
      Expr targetExpr = computeTargetExpr(filter, tid, analyzer);
      if (targetExpr == null) continue;
      boolean isLocalTarget = isLocalTargetForPlanNode(filter, cteNode);
      if (runtimeFilterMode == TRuntimeFilterMode.LOCAL && !isLocalTarget) continue;
      RuntimeFilter.RuntimeFilterTarget target =
          new RuntimeFilter.RuntimeFilterTarget(cteNode, targetExpr, isLocalTarget);
      filter.addTarget(target);
    }
  }
