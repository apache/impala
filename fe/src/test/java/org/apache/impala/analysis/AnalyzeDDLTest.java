...
public class AnalyzeDDLTest extends FrontendTestBase {
    ...
    public void TestAlterTableAddDropPartition() throws CatalogException {
        ...
        // Duplicate date partition key name. Date literals may have different
        // variations.
        AnalysisError(
        ...
        String paimon_partitioned = "ALTER TABLE functional_parquet.paimon_partitioned";
    ...
    public void TestAlterTableSetRowFormat() throws AnalysisException {
        ...
        AnalysisError("alter table functional.alltypesnopart PARTITION (month=1) " +
                ...
                "Table is not partitioned: functional.alltypesnopart");
        String[] unsupportedFileFormatDbs = { "functional_parquet", "functional_rc", "functional_avro" };
        for (String format : unsupportedFileFormatDbs) {
            AnalysisError("alter table " + format + ".alltypes set row format delimited " +
                    "fields terminated by ' '",
                    "ALTER TABLE SET ROW FORMAT is only supported " +
    ...
    public void TestAlterTableSet() throws AnalysisException {
...

        {
            ...
            // Now that long_property_{key,value} are one character longer, they are too
            // long
            // for the analyzer.
        ...
        // Arbitrary exprs as partition key values. One-partition-column-bound exprs are
        // ok.
        AnalyzesOk("alter table functional.alltypes PARTITION " +
        ...
        String tmpTableName = QueryStringBuilder.createTmpTableName("functional", "tmp_table");
        ...
        for (String format : unsupportedFileFormatDbs) {
            AnalysisError("alter table " + format + ".alltypes set serdeproperties(" +
                    "'serialization.encoding'='GBK')",
                    "Property 'serialization.encoding' is " +
        ...
        AnalysisError("alter table functional_kudu.alltypes set serdeproperties( " +
                "'serialization.encoding'='GBK')",
                "Property 'serialization.encoding' is only " +
        ...
        // IMPALA-12918: Validate numeric table stats properties
    ...
    public void TestAlterView() {
        ...
        // Mismatching number of columns in column definition and view-alteration
        // statement.
        ...
        AnalysisError("alter view functional.alltypes_view " +
                "as select null as new_col",
                "Unable to infer the column type for " +
    ...
    /**
     * Analyzes 'stmt' and checks that the table-level and column-level SQL that is
     * used
    ...
    ComputeStatsStmt checkComputeStatsStmt(String stmt, AnalysisContext ctx,
        ...
        if (colsQuery != null)
            AnalyzesOk(colsQuery);
        return parsedStmt;
    }

    /**
     * In addition to the validation for checkComputeStatsStmt(String), checks that
     * the
    ...
    void checkComputeStatsStmt(String stmt, List<String> expColNames)
        ...
        if (expColNames == null)
            assertTrue("Expected no whitelist.", actCols == null);
    ...
    public void TestComputeStats() throws AnalysisException {
        ...
        // See testdata/avro_schema_resolution/create_table.sql for the CREATE TABLE
        // stmts.
        ...
        // Test tablesample clause with extrapolation enabled/disabled. Replace/restore
        // the
        ...
        try {
            ...
            // The baseline statement with no sampling minimum should select exactly one
            // file
    ...
    public void TestComputeIncrementalStats() throws AnalysisException {
        ...
        AnalysisError("compute incremental stats functional_hbase.alltypes " +
                ...
                "COMPUTE INCREMENTAL STATS ... PARTITION not " +
    ...
    public void TestDropIncrementalStats() throws AnalysisException {
        ...
        AnalysisError("drop incremental stats functional_hbase.alltypes "
                + "partition(year=2010, month=1)",
                "DROP INCREMENTAL STATS ... PARTITION "
    ...
    public void TestTruncate() throws AnalysisException {
        ...
        // If the database exists but the table doesn't, an analysis error should be
        // thrown.
    ...
    public void TestCreateTableLikeFileOrcWithHive2() throws AnalysisException {
        // Testing if error is thrown when trying to create table like orc file with
        // Hive-2.
    ...
    public void TestCreateTableAsSelect() throws AnalysisException {
        ...
        AnalysisError("create external table t stored as kudu " +
                ...
                "CREATE TABLE AS SELECT is not supported for " +
    ...
    public void TestCreateTableLike() throws AnalysisException {
        ...
        AnalysisError("create table functional.`^&*` like functional.alltypes",
                "Invalid table/view name: ^&*");
        // Invalid source database/table name reports non-existence instead of
        // invalidity.
        ...
        // 'file' scheme does not take an authority, so file:/// is equivalent to
        // file://
        // and file:/.
        AnalyzesOk("create table tbl like functional.alltypes location " +
        ...
        // CREATE TABLE LIKE is only implements cloning between Kudu tables (see
        // IMPALA-4052)
    ...
    public void TestCreateTable() throws AnalysisException {
        ...
        AnalysisError("create table fk(id int, year string, foreign key(id, year) "
                + "references pk(year))", ctx,
                "The number of foreign key columns should be same"
        ...
        AnalysisError("create table fk(id int, foreign key(id) references "
                + "functional.alltypes(int_col))", ctx,
                "Parent column int_col is not part of "
                        + "primary key.");
        ...
        for (String format : fileFormats) {
            ...
            AnalysisError(String.format("create table t (i int primary key) stored as %s",
                    format),
                    String.format("Unsupported column options for file format " +
        ...
        AnalysisError("create table Foo (i int) sort by (i) " +
                "tblproperties ('sort.columns'='i')",
                "Table definition must not contain the " +
        ...
        AnalysisError("create table Foo (i int, j int) sort by zorder (i, j) " +
                "tblproperties ('sort.order'='ZORDER')",
                "Table definition must not " +
        ...
        AnalysisError("create table functional.new_table (i int) PARTITIONED BY (d decimal)" +
                "sort by zorder (i, d)",
                "SORT BY column list must not contain partition " +
                        "column: 'd'");
    }
    ...
    public void TestCreateBucketedTable() throws AnalysisException {
        ...
        AnalysisError("CREATE TABLE functional.bucket (i int COMMENT 'hello', s string) " +
                "CLUSTERED BY (i) INTO 24 BUCKETS STORED BY KUDU",
                "CLUSTERED BY not " +
    ...
    public void TestCreateAvroTest() {
        ...
        AnalysisError("create table functional.new_table (i int primary key) " +
                "stored as avro",
                "Unsupported column options for file format 'AVRO': " +
                        "'i INT PRIMARY KEY'");
    }
    ...
    public void TestCreateView() throws AnalysisException {
        ...
        // Mismatching number of columns in column definition and view-definition
        // statement.
        ...
        AnalyzesOk("create view functional.foo (a) as " +
                "select int_map_col " +
                "from functional.allcomplextypes");
        // It's allowed to do the same with struct as it is supported in the select
        // list.
    ...
    public void TestUdf() throws AnalysisException {
        ...
        // Add functions default.TestFn(), default.TestFn(double),
        // default.TestFn(String...),
    ...
    public void TestUda() throws AnalysisException {
        ...
        String symbols = " UPDATE_FN='_Z9AggUpdatePN10impala_udf15FunctionContextERKNS_6IntValEPS2_' " +
        ...
        // Test cases where the UPDATE_FN doesn't contain "Update" in which case the
        // user has
        ...
        // If you don't specify the full symbol, we look for it in the binary. This
        // should
     ...
     * Also tests that the type is analyzes correctly in ARRAY, MAP, and STRUCT
     * types.
     */
    ...
    public void TestTypes() {
        ...
        for (String prefix : Arrays.asList("struct<f1:int,f2:", "array<", "map<string,")) {
            ...
            String oneAboveMaxDef = genTypeSql(Type.MAX_NESTING_DEPTH + 1, prefix, middle, suffix);
            ...
            String veryDeepDef = genTypeSql(Type.MAX_NESTING_DEPTH * 100, prefix, middle, suffix);
    ...
    public void TestDescribe() throws AnalysisException {
        ...
        // Path could be resolved as either <db>.<table>.<field> or
        // <table>.<field>.<field>
    ...
    public void TestShowFiles() throws AnalysisException {
        ...
        String icebergPartitioned = "show files in functional_parquet.iceberg_partitioned";
        String icebergEvolution = "show files in functional_parquet.iceberg_partition_evolution";
        String icebergNonPartitioned = "show files in functional_parquet.iceberg_non_partitioned";
    ...
    public void TestShowPartitions() throws AnalysisException {
        ...
        // WHERE clause with Kudu table should fail (non-HDFS tables don't support
        // WHERE)
    ...
    public void TestCommentOnDatabase() {
        ...
        AnalysisError(String.format("comment on database functional is '%s'",
                buildLongComment()),
                "Comment exceeds maximum length of 256 characters. " +
    ...
    public void TestAlterDatabaseSetOwner() {
        ...
        for (String ownerType : ownerTypes) {
            ...
            AnalysisError(String.format("alter database functional set owner %s %s",
                    ownerType, buildLongOwnerName()),
                    "Owner name exceeds maximum length of 128 " +
    ...
    public void TestAlterExecuteExpireSnapshots() {
        ...
        AnalysisError("alter table functional_parquet.iceberg_partitioned execute " +
                "unsupported_operation(123456789);",
                "'unsupported_operation' is not supported " +
                        "by ALTER TABLE <table> EXECUTE. Supported operations are: " +
                        "EXPIRE_SNAPSHOTS(<expression>), ROLLBACK(<expression>)");
        AnalysisError("alter table functional_parquet.iceberg_partitioned execute " +
                "expire_snapshots(now(), 3);",
                "EXPIRE_SNAPSHOTS(<expression>) must have one " +
                        "parameter: expire_snapshots(now(), 3)");
        AnalysisError("alter table functional_parquet.iceberg_partitioned execute " +
                "expire_snapshots(id);",
                "EXPIRE_SNAPSHOTS(<expression>) must be a constant " +
                        "expression: expire_snapshots(id)");
        AnalysisError("alter table functional_parquet.iceberg_partitioned execute " +
                "expire_snapshots(42);",
                "EXPIRE_SNAPSHOTS(<expression>) must be a timestamp " +
                        "type but is 'TINYINT': 42");
        AnalysisError("alter table functional_parquet.iceberg_partitioned execute " +
                "expire_snapshots('2021-02-32 15:52:45');",
                "Invalid TIMESTAMP expression has" +
