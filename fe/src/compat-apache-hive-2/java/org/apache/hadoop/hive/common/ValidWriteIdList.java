...
// Original Source:
// https://github.com/apache/hive/tree/branch-3.1/storage-api/src/java/
//       org/apache/hadoop/hive/common/ValidWriteIdList.java
...
public interface ValidWriteIdList {
  ...
  /**
   * Indicates whether a given write ID is valid. Note that valid may have different
   * meanings for different implementations, as some will only want to see
   * committed transactions and some
  ...
  /**
   * Returns {@code true} if such base file can be used to materialize the snapshot
   * represented by
  ...
  /**
   * Find out if a range of write ids are valid.  Note that valid may have different
   * meanings for different implementations, as some will only want to see committed
