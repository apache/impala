...
class ImpalaCluster(object):
  ...
  def __load_hosts(self):
    ...
    for host_info in self.cm_api.get_all_hosts():
      # host_info doesn't include a like to the roleRef so need to do another lookup
      # based on the hostId.
      host = self.cm_api.get_host(host_info.hostId)
      for roleRef in host.roleRefs:
        if 'clusterName' in roleRef:
          cluster_name = roleRef['clusterName']
          if cluster_name == self.cluster.name:
